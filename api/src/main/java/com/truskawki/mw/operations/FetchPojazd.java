...
public class FetchPojazd extends DatabaseComplexResponseOperation {
    ...
    protected TruskawkiSimpleResponse mainAction() {
        ...
        try{
            pojazd = ((VehicleMapper) mapper).getPojazd(vin);
            String rodzaj_pojazdu = ((VehicleMapper) mapper).getRodzaj_pojazdu(vin);
            String marka = ((VehicleMapper) mapper).getMarka(vin);
